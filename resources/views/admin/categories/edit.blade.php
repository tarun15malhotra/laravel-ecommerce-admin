...
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form action="{{ route('admin.categories.update', $category ?? 1) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                    ...
                    <div class="bg-white shadow rounded-lg p-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="name" id="name" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" value="{{ old('name', $category->name ?? 'Electronics') }}">
                                @error('name')
                            ...
                            <div>
                                ...
                                <input type="text" name="slug" id="slug" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" value="{{ old('slug', $category->slug ?? 'electronics') }}">
                            ...
                            <div>
                                ...
                                <select name="parent_id" id="parent_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    ...
                                    @foreach($categories ?? [] as $cat)
                                        @if($cat->id != ($category->id ?? 0))
                                            <option value="{{ $cat->id }}" {{ old('parent_id', $category->parent_id ?? null) == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                                        @endif
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="sort_order" id="sort_order" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" value="{{ old('sort_order', $category->sort_order ?? 0) }}">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white shadow rounded-lg p-6">
                        ...
                        <label class="flex items-center">
                            <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" {{ old('is_active', $category->is_active ?? true) ? 'checked' : '' }}>
                    ...
                    <div class="bg-white shadow rounded-lg p-6">
                        <h2 class="text-lg font-medium text-gray-900 mb-4">Current Image</h2>
                        <img src="{{ Storage::url($category->image) }}" alt="Category image" class="w-full h-32 object-cover rounded">
                    </div>
                    @endif
